...
APP_DATA_DIR = Path(os.environ.get('APPDATA', os.path.expanduser('~'))) / APP_NAME
CONFIG_FILE = APP_DATA_DIR / "config.json"
DATABASE_FILE = APP_DATA_DIR / "bookstore.db"
...
def initDatabase():
    try:
        ...
        try:
            ...
            basePath = Path(__file__).parent
        dataSQL = basePath / 'data.sql'

        with open(dataSQL, 'r', encoding = 'utf-8') as data:
            connection.executescript(data.read())

        #do the offset time fixing here
        
        offset = (date.today() - date(2026, 6, 25)).days

        cursor = connection.cursor()
        cursor.execute(
            "UPDATE sales SET sale_at = datetime(sale_at, ?)",
            (f"{offset} days",)
        )

        cursor.execute(
            """UPDATE borrowings
            SET borrow_at = datetime(borrow_at, ?),
                due_on = datetime(due_on, ?)""",
            (f"{offset} days", f"{offset} days")
        )
        ...
        cursor.execute(
            ...
            SET borrow_at = datetime(borrow_at, ?),
                due_on = datetime(due_on, ?),
                return_at = datetime(return_at, ?)""",
            (f"{offset} days", f"{offset} days", f"{offset} days")
        )
...
def configureSetting():
    ...
    lowStock = int(inquirer.text(
        message = "LOW_STOCK: ",
        default = config['LOW_STOCK'],
        validate = isIntP,
        invalid_message = "Must be positive integer",
        style = MENU_STYLE
    ).execute())
    finePerDay = float(inquirer.text(
        message = "FINE_PER_DAY: ",
        default = config['FINE_PER_DAY'],
        validate = isFloatP,
        invalid_message = "Must be positive number",
        style = MENU_STYLE
    ).execute())
    borrowFeePerDay = float(inquirer.text(
        message = "BORROW_FEE_PER_DAY: ",
        default = config['BORROW_FEE_PER_DAY'],
        validate = isFloatP,
        invalid_message = "Must be positive number",
        style = MENU_STYLE
    ).execute())
    ...
    if saveConfig(setting):
        print("Settings Updated Sucesfully")
    else:
...
def validateSettings():
    ...
    if not (config['LOW_STOCK']>=0 and config['FINE_PER_DAY']>=0 and config['BORROW_FEE_PER_DAY']>=0):
        configureSetting()
        return
...
class BookStore() :
    ...
    def connect(self):
        try:
            ...
            # detect_type fixes the timestamp being string problem
            self.connection = sqlite3.connect(str(DATABASE_FILE), detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
            # fix forign keys
    ...
    def addCustomer(self):
        ...
        existing = self.executeQuery("""SELECT customer_id FROM customers WHERE  email = ?""",
                                     (email,), fetch = True)
        ...
        if self.executeQuery("""INSERT INTO customers (name, email, phone) VALUES (?, ?, ?)""",
                             (name, email, phone)):
    ...
    def searchCustomer(self):
        ...
        search_options = {
            ...
            'Email': {
                ...
                'query': "SELECT * FROM customers WHERE email LIKE ? ORDER BY name",
                'transform': lambda x : f"%{x}%",
                'validate': lambda x: True,
                'invalid_message':""
            },
            ...
            'Customer ID': {
                ...
                'query': "SELECT * FROM customers WHERE customer_id = ? ORDER BY name",
                'transform': lambda x : f"{x}",
                'validate': isIntP,
    ...
    def updateCustomers(self):
        ...
        customer = self.executeQuery("SELECT * FROM customers WHERE customer_id = ?",(customerId,), fetch = True)
        if not customer:
    ...
    def deleteCustomer(self):
        ...
        if self.executeQuery("DELETE FROM customers WHERE customer_id = ?",
                             (customerId,)):
    ...
    def searchBook(self):
        ...
        search_options = {
            ...
            'Genre': {
                ...
                'query': "SELECT * FROM books WHERE genre LIKE ? ORDER BY title",
                'transform': lambda x : f"%{x}%",
                'validate': lambda x: True,
                'invalid_message': ""
            },
            ...
            'Book ID': {
                ...
                'query': "SELECT * FROM books WHERE book_id = ? ORDER BY title",
                'transform': lambda x : f"{x}",
                'validate': isIntP,
                'invalid_message': "Please enter valid ISBN"
            },
    ...
    def updateBook(self):
        ...
        try:
            ...
            return
        book = self.executeQuery("SELECT * FROM books WHERE book_id = ?",(bookId,), fetch = True)
        if not book:
            print("Book not found")
            return
        ...
        if self.executeQuery("""UPDATE books 
                             SET title = ?, author = ?, price = ?, quantity = ?, genre = ?
                             WHERE book_id = ?""",
    ...
    def deleteBook(self):
        ...
        book = self.executeQuery("""SELECT title FROM books WHERE book_id = ?""",
                                 (bookId,), fetch = True)
        if not book:
            print("Book not found")
            return
        ...
        if self.executeQuery("DELETE FROM books WHERE book_id = ?",
                             (bookId,)):
    ...
    def sellBook(self):
        ...
        try:
            book = self.executeQuery("SELECT * FROM books WHERE book_id = ?",
                                    (bookId,), fetch = True)
        except Error as e:
        ...
        try: 
            self.cursor.execute("UPDATE books SET quantity = quantity - ? WHERE book_id = ?",
                                (quantitySold, bookId))
    ...
    def viewRevenueReport(self):
        print("\n=== COMPRENSIVE SALES REPORT ===")
                                    ...
                                    # and this DATE(s.sale_at) >= DATE('now', 'start of month') makes it
                                    # so that we only se this month's reports 
                                    # SQLite dose it better than MySQL
        bookSales = self.executeQuery("""SELECT s.sale_at as sale_date, b.title, b.author, c.name as customer_name, s.quantity_sold, 
                                      ...
                                      WHERE DATE(s.sale_at) >= DATE('now', 'start of month')
        ...
        borrowingRevenue = self.executeQuery("""SELECT r.return_at as sale_date, b.title, b.author, 
                                             ...
                                             WHERE DATE(r.return_at) >= DATE('now', 'start of month')
        ...
        for transaction in allTransactions:
            ...
            if transaction['transaction_type'] == 'Book Sale': 
                totalSalesRevenue += float(transaction['total_amount'])
            else:
                totalBorrowingRevenue += float(transaction['total_amount'])
    ...
    def viewRevenueReportSeperate(self):
        ...
        borrowingRevenue = self.executeQuery("""SELECT r.return_at, b.title, b.author, 
                                             ...
                                             CAST(julianday(r.return_at) - julianday(r.due_on) AS INTEGER) as days_overdue
                                             FROM returns r
                                             JOIN books b ON r.book_id = b.book_id
                                             JOIN customers c ON r.customer_id = c.customer_id
                                             WHERE DATE(r.return_at) >= DATE('now', 'start of month')
    ...
    def borrowBook(self):
        ...
        try:
            self.cursor.execute("UPDATE books SET quantity = quantity - 1 WHERE book_id = ?",
                                (bookId,))
    ...
    def returnBook(self):
        ...
        if returnDate > dueDate:
            ...
            fineAmount = daysOverdue * self.config["FINE_PER_DAY"]
            print(f"Book is {daysOverdue} days overdue. Fine: ₹{fineAmount:.2f}")
        else:
            fineAmount = 0

        totalAmount = (dueDate - borrowDate).days * self.config["BORROW_FEE_PER_DAY"]
        try:
            ...
            self.cursor.execute("DELETE FROM borrowings WHERE borrowing_id = ?",
                                (borrowId,))
            self.cursor.execute("UPDATE books SET quantity = quantity + 1 WHERE book_id = ?",
    ...
    def viewBorrowedBook(self):
        ...
        borrowedBooks = self.executeQuery("""SELECT bb.borrowing_id, c.name, bb.borrow_at, bb.due_on, 
                                          b.title, b.author, b.isbn, MAX(CAST(julianday('now') - julianday(bb.due_on) AS INTEGER), 0) as days_overdue
    ...
    def viewLowStock(self):
        ...
        books = self.executeQuery("SELECT * FROM books WHERE quantity < ? ORDER BY quantity ASC",
                                  (self.config["LOW_STOCK"],), fetch = True)
...
def main():
    ...
    if not bookstore.connection:
        print("Failed to connect. Please check the SQLite configurations")
        return
